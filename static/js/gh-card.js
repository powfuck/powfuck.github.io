...
const themes = {
  ...
  "dark-theme": {
    background: "rgb(29, 30, 32)", // default dark theme for hugo theme
...
const getTheme = function () {
  if (localStorage.getItem("pref-theme") === "dark") {
    ...
    return themes["light-default"];
  } else if (window.matchMedia("(prefers-color-scheme: dark)").matches) {
    return themes["dark-theme"];
  }
};

// follow change theme when toggle clicked
document.getElementById("theme-toggle").addEventListener("click", () => {
  const repocard = document.querySelector("#repo-card").querySelectorAll("div");
  repocard.forEach((data, index) => {
    if (document.body.className.includes("dark")) {
      data.style.borderColor = themes["light-default"].borderColor;
      data.style.background = themes["light-default"].background;
      data.style.backgroundColor = themes["light-default"].backgroundColor;
      data.style.color = themes["light-default"].color;
      data.style.linkColor = themes["light-default"].linkColor;
    } else {
      data.style.borderColor = themes["dark-theme"].borderColor;
      data.style.background = themes["dark-theme"].background;
      data.style.backgroundColor = themes["dark-theme"].backgroundColor;
      data.style.color = themes["dark-theme"].color;
      data.style.linkColor = themes["dark-theme"].linkColor;
    }
  });
});
...
window.addEventListener("DOMContentLoaded", async function () {
  if (/\/project/.test(window.location.href)) {
    const parrent = document.getElementById("repo-card");
    parrent.insertAdjacentHTML(
      "beforeend",
      `<img
    id="menhera"
    src="/images/menhera-chibi.gif"
    width="50%"
    height="50%"
  />`
    );
    ...
    repos
      ...
      .forEach((data, index) => {
        ...
        parrent.insertAdjacentHTML(
          "beforeend",
          `
    <div class="anonim" style="margin-bottom: 10px; font-family: -apple-system,BlinkMacSystemFont,Segoe UI,Helvetica,Arial,sans-serif,Apple Color Emoji,Segoe UI Emoji; border: 1px solid ${
      theme.borderColor
      ...
      <div style="display: ${
        data.fork ? "block" : "none"
            ...
            theme.color
          };">Forked from <a style="color: inherit; text-decoration: none;" target="_blank" href="${
    ...
    const menhera = document.getElementById("menhera");
    if (menhera != null) {
      parrent.removeChild(menhera);
    }
  }
});
